...
const nameSchema: ParamSchema = {
  ...
  isLength: {
    ...
    errorMessage: USER_MESSAGES.NAME_LENGHT_MUST_BE_FROM_1_TO_100
  }
}
  ...
  checkSchema(
    {
      email_verify_token: {
        ...
        isString: {
          errorMessage: USER_MESSAGES.EMAIL_VERIFY_TOKEN_MUST_BE_A_STRING
        },
        custom: {
          options: async (value: string, { req }) => {
            try {
      ...
      username: {
        ...
        custom: {
          options: async (value, { req }) => {
            ...
            if (user) {
              throw Error(USER_MESSAGES.USERAME_EXISTED)
            }
          }
        }
      },
...
export const isUserLoggedInValidator = (middlware: (req: Request, res: Response, next: NextFunction) => void) => {
